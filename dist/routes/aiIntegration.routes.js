...
router.get("/accidents", roleMiddleware([UserRole.POLICE, UserRole.FIRE_SERVICE, UserRole.ADMIN]), AIIntegrationController.getAccidentData);
// Get specific accident by ID (available to police, fire service, and admin)
router.get("/accidents/:accidentId", roleMiddleware([UserRole.POLICE, UserRole.FIRE_SERVICE, UserRole.ADMIN]), AIIntegrationController.getAccidentById);
// Send accident alert to AI service (available to police, fire service, and admin)
router.post("/alerts", roleMiddleware([UserRole.POLICE, UserRole.FIRE_SERVICE, UserRole.ADMIN]), AIIntegrationController.sendAccidentAlert);
// Process image/video link for detection (available to police, fire service, and admin)
router.post("/detect", roleMiddleware([UserRole.POLICE, UserRole.FIRE_SERVICE, UserRole.ADMIN]), AIIntegrationController.processMediaForDetection);
...
router.post("/sync", roleMiddleware([UserRole.ADMIN]), AIIntegrationController.syncAccidentData);
