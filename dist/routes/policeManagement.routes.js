 ...
 */
router.get("/pending-reports", roleMiddleware(["POLICE", "ADMIN", "SUPER_ADMIN"]), CitizenReportsController.getPendingReports);
/**
 * @route GET /api/police/review-stats
 * @desc Get review statistics for police dashboard
 * @access Private (Police/Admin/Super Admin only)
 */
router.get("/review-stats", roleMiddleware(["POLICE", "ADMIN", "SUPER_ADMIN"]), CitizenReportsController.getReviewStats);
/**
 * @route GET /api/police/pending-appeals
 * @desc Get pending appeals for review
 * @access Private (Police/Admin/Super Admin only)
 */
router.get("/pending-appeals", roleMiddleware(["POLICE", "ADMIN", "SUPER_ADMIN"]), CitizenReportsController.getPendingAppeals);
/**
 * @route POST /api/police/review-appeal/:reportId
 * @desc Review appeal (approve/reject)
 * @access Private (Police/Admin/Super Admin only)
 */
router.post("/review-appeal/:reportId", roleMiddleware(["POLICE", "ADMIN", "SUPER_ADMIN"]), CitizenReportsController.reviewAppeal);
/**
 * @route POST /api/police/review/:reportId
 * @desc Review citizen report (approve/reject)
 * @access Private (Police/Admin/Super Admin only)
 */
router.post("/review/:reportId", roleMiddleware(["POLICE", "ADMIN", "SUPER_ADMIN"]), CitizenReportsController.reviewReport);
