...
function decryptMessage(text) {
    try {
        ...
        if (iv.length !== IV_LENGTH) {
            console.warn("Invalid IV length in official chat decryption, returning original text");
            return text;
        }
...
class OfficialChatService {
     ...
     */
    static async createOrGetChatRoom(user1Id, user2Id) {
        ...
        if (!officialRoles.includes(user1.role) ||
            !officialRoles.includes(user2.role)) {
            throw new Error("Both users must be officials to create an official chat");
        }
        if (!user1.isActive ||
            user1.isBlocked ||
            !user2.isActive ||
    ...
    static async markMessageAsRead(messageId, userId) {
        ...
        if (message.chatRoom.user1Id !== userId &&
     ...
     */
    static async getAvailableOfficials(userId) {
