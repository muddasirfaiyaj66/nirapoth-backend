...
class TrafficController {
     ...
     */
    static async getNearbyTraffic(req, res) {
        try {
            ...
            const trafficJams = analyzeTrafficDensity(recentViolations, latitude, longitude);
            res.json({
                success: true,
                data: {
                    trafficJams,
     ...
     * Public endpoint - no auth required
     */
    static async getAccidentHotspots(req, res) {
        try {
            const validation = nearbyTrafficSchema.safeParse(req.query);
            if (!validation.success) {
                res.status(400).json({
            ...
            const hotspots = identifyAccidentHotspots(violations, latitude, longitude);
            res.json({
                success: true,
                data: {
                    hotspots,
     ...
     * Public endpoint - no auth required
     */
    static async calculateSafeRoute(req, res) {
        try {
            ...
            const routeOptions = await analyzeRouteOptions({ lat: originLat, lng: originLng }, { lat: destLat, lng: destLng });
            res.json({
                success: true,
                data: {
                    routes: routeOptions,
...
function analyzeTrafficDensity(violations, centerLat, centerLng) {
    ...
    violations.forEach((v) => {
        if (v.location) {
            const key = `${v.location.latitude.toFixed(3)},${v.location.longitude.toFixed(3)}`;
...
function identifyAccidentHotspots(violations, centerLat, centerLng) {
    // Group by location clusters (within 0.5km)
    const clusters = {};
    violations.forEach((v) => {
        if (v.location) {
            const clusterKey = `${Math.floor(v.location.latitude * 20) / 20},${Math.floor(v.location.longitude * 20) / 20}`;
    ...
    Object.entries(clusters).forEach(([key, group]) => {
        if (group.length >= 10) {
            ...
            const avgLat = group.reduce((sum, v) => sum + v.location.latitude, 0) / group.length;
            const avgLng = group.reduce((sum, v) => sum + v.location.longitude, 0) / group.length;
...
async function analyzeRouteOptions(origin, dest) {
    // In production, call Google Maps Directions API with alternatives
    // Then analyze each route for traffic and accidents
    const directDistance = calculateDistance(origin.lat, origin.lng, dest.lat, dest.lng);
    return [
        {
            id: "route-1",
            name: "Fastest Route",
