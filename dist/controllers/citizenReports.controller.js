...
exports.CitizenReportsController = void 0;
...
class CitizenReportsController {
            ...
            if (reviewedReports.length > 0) {
                const totalMinutes = reviewedReports.reduce((sum, report) => {
                    if (report.reviewedAt) {
                        const diffMs = new Date(report.reviewedAt).getTime() -
    ...
    static async createReport(req, res) {
        try {
            const userId = req.user?.id;
            const { vehiclePlate, violationType, description, evidenceUrls, locationData, } = req.body;
            // Validation
            if (!vehiclePlate || !violationType || !evidenceUrls || !locationData) {
                res.status(400).json({
                    success: false,
                    message: "Missing required fields: vehiclePlate, violationType, evidenceUrls, locationData",
                });
                return;
            }
    ...
    static async submitAppeal(req, res) {
        try {
            ...
            if (report.appealSubmitted) {
                res.status(400).json({
                    success: false,
                    message: "This report has already been appealed. Appeals can only be submitted once.",
                });
                return;
            }
            ...
            res.status(200).json({
                success: true,
                message: "Appeal submitted successfully. You will be notified of the decision within 7 days.",
                data: updatedReport,
            });
        }
        catch (error) {
    ...
    static async reviewAppeal(req, res) {
        try {
            ...
            const message = action === "APPROVED"
                ...
                : `Appeal rejected. Additional penalty of ${additionalPenalty.toFixed(2)} BDT applied.`;
            res.status(200).json({
                success: true,
                message,
                data: updatedReport,
...
exports.CitizenReportsController = CitizenReportsController;
