 ...
 */
export const getFireServiceStats = async (req, res) => {
    try {
        ...
        const stats = {
            ...
            recentEmergencies: recentEmergencies.map((e) => ({
                id: e.id,
                location: `${e.location?.area || ""}, ${e.location?.district || ""}`.trim(),
                severity: e.severity,
                status: e.status,
                date: e.createdAt,
 ...
 */
export const getFireServiceAnalytics = async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({
    ...
    `;
        ...
        console.log("🔥 Emergencies by month:", analytics.emergenciesByMonth.length);
