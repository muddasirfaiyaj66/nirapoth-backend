...
export class VehicleController {
    ...
    static async getVehicleById(req, res) {
        try {
            ...
            const hasAccess = userRole === UserRole.ADMIN ||
                ...
                vehicle.assignments.some((assignment) => assignment.citizenId === userId);
            if (!hasAccess) {
    ...
    static async addVehicle(req, res) {
        try {
            const userId = req.user?.userId;
            const { plateNo, registrationNo, engineNo, chassisNo, brand, model, year, color, type, expiresAt, } = req.body;
            ...
            if (!plateNo ||
                ...
                return res.status(400).json({
                    success: false,
                    message: "Missing required fields: plateNo, engineNo, chassisNo, brand, model, year, type",
                });
            }
    ...
    static async updateVehicle(req, res) {
        try {
            ...
            const canUpdate = userRole === UserRole.ADMIN || vehicle.ownerId === userId;
            if (!canUpdate) {
                return res.status(403).json({
                    success: false,
                    message: "Access denied",
    ...
    static async deleteVehicle(req, res) {
        try {
            ...
            const canDelete = userRole === UserRole.ADMIN || vehicle.ownerId === userId;
            if (!canDelete) {
                return res.status(403).json({
                    success: false,
                    message: "Access denied",
    ...
    static async assignSelfAsDriver(req, res) {
        try {
            const { vehicleId } = req.params;
            const userId = req.user?.userId;
