...
export class CitizenGemService {
   ...
   */
  static async updateCitizenGems(citizenId: string, newAmount: number) {
   ...
   */
  static async decreaseGems(citizenId: string, decreaseBy: number) {
   ...
   */
  static async increaseGems(citizenId: string, increaseBy: number) {
    const currentGem = await prisma.citizenGem.findUnique({
      where: { citizenId },
    });
