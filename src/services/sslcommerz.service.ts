...
export class SSLCommerzService {
   ...
   */
  async initPayment(data: InitPaymentData): Promise<any> {
    try {
      const paymentData = {
   ...
   */
  async validatePayment(validationData: ValidationData): Promise<any> {
    try {
  ...
  async initiateRefund(
    ...
    try {
      ...
      const response = await axios.post(
        ...
        new URLSearchParams(refundData as any).toString(),
        {
          headers: {
            "Content-Type": "application/x-www-form-urlencoded",
          },
   ...
   */
  generateTransactionId(prefix: string = "NIRAPOTH"): string {
   ...
   */
  async createFinePaymentSession(
    fineId: string,
    userId: string,
    amount: number,
    baseURL: string
   ...
   */
  async createMultipleFinesPaymentSession(
    fineIds: string[],
    userId: string,
    baseURL: string
  ): Promise<any> {
    try {
      ...
      console.error(
        "Create multiple fines payment session error:",
        error.message
      );
      return {
        success: false,
   ...
   */
  async createDebtPaymentSession(
    debtId: string,
    userId: string,
    amount: number,
    baseURL: string
    ...
    try {
      ...
      const transactionId = `DEBT_${debtId}_${Date.now()}_${Math.floor(
        Math.random() * 10000
      )}`;
   ...
   */
  async handleSuccessCallback(data: any): Promise<any> {
    try {
      // Validate payment
      ...
      if (payment.user && payment.fine) {
        try {
          await emailService.sendPaymentConfirmation({
            ...
            fineDetails: {
              violationType:
                payment.fine.violation?.rule?.title || "Traffic Violation",
   ...
   */
  async handleFailCallback(data: any): Promise<any> {
    try {
      const transactionId = data.tran_id;
   ...
   */
  async handleCancelCallback(data: any): Promise<any> {
    try {
      const transactionId = data.tran_id;
