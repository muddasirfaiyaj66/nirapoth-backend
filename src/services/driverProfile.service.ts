...
export class DriverProfileService {
   ...
   */
  static async createDriverProfile(
    data: CreateDriverProfileData
  ): Promise<any> {
   ...
   */
  static async getDriverProfileById(profileId: string): Promise<any> {
    const profile = await prisma.driverProfile.findUnique({
      where: { id: profileId },
      include: {
        user: {
   ...
   */
  static async getDriverProfileByUserId(userId: string): Promise<any> {
    const profile = await prisma.driverProfile.findUnique({
      where: { userId },
      include: {
        user: {
   ...
   */
  static async searchDrivers(filters: DriverSearchFilters): Promise<any[]> {
    ...
    if (filters.minGems !== undefined) {
      filteredProfiles = profiles.filter(
        (profile) => (profile.user?.citizenGem?.amount || 0) >= filters.minGems!
      );
    }
   ...
   */
  static async deleteDriverProfile(
    profileId: string,
    userId: string
  ): Promise<void> {
    // Verify ownership
