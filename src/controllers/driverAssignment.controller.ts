...
export const createAssignment = async (
  ...
  try {
    ...
    await NotificationService.notifyVehicleAssignmentCreated(
      assignment.driverId,
      ownerName,
...
export const acceptAssignment = async (
  ...
  try {
    ...
    const { id } = req.params;
    const assignment = await DriverAssignmentService.acceptAssignment(
      id,
      userId
    );
...
export const rejectAssignment = async (
  ...
  try {
    ...
    const { id } = req.params;
    const assignment = await DriverAssignmentService.rejectAssignment(
      id,
      userId
    );
...
export const resignFromAssignment = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    const { id } = req.params;
    const assignment = await DriverAssignmentService.resignFromAssignment(
      id,
      userId
    );
    ...
    // Send notification to owner
    const driverName = `${assignment.driver.firstName} ${assignment.driver.lastName}`;
    await NotificationService.notifyDriverResigned(
      assignment.ownerId,
      driverName,
      assignment.vehicle.plateNo,
      assignment.id
...
export const terminateAssignment = async (
  ...
  try {
    ...
    const { id } = req.params;
    const assignment = await DriverAssignmentService.terminateAssignment(
      id,
      userId
    );
    ...
    // Send notification to driver
    const ownerName = `${assignment.owner.firstName} ${assignment.owner.lastName}`;
    await NotificationService.notifyAssignmentTerminated(
      assignment.driverId,
      ownerName,
      assignment.vehicle.plateNo
    ).catch((err) => console.error("Failed to send notification:", err));
...
export const getAssignment = async (
  ...
  try {
    ...
    res
      .status(
        error.message === "Unauthorized access to this assignment" ? 403 : 404
      )
      .json({
        success: false,
...
export const getVehicleAssignments = async (
  ...
  try {
    ...
    const assignments = await DriverAssignmentService.getVehicleAssignments(
      vehicleId,
      userId
    );
    ...
    res
      .status(
        error.message === "Unauthorized access to vehicle assignments"
          ? 403
          : 404
      )
      .json({
...
export const getDriverAssignments = async (
  ...
  try {
    ...
    const assignments = await DriverAssignmentService.getDriverAssignments(
      driverId
    );
