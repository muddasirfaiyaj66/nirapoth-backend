...
export class CameraController {
   ...
   */
  static async getCameras(req: AuthRequest, res: Response): Promise<void> {
    try {
   ...
   */
  static async getCameraById(req: AuthRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
   ...
   * Create a new camera
   */
  static async createCamera(req: AuthRequest, res: Response): Promise<void> {
    try {
      const { name, streamUrl, status, locationId, stationId, fireServiceId } =
        req.body;
   ...
   */
  static async updateCamera(req: AuthRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const { name, streamUrl, status, locationId, stationId, fireServiceId } =
        req.body;
   ...
   * Delete a camera
   */
  static async deleteCamera(req: AuthRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
   ...
   */
  static async getCameraStats(req: AuthRequest, res: Response): Promise<void> {
    try {
    ...
    try {
      ...
      if (!["ACTIVE", "INACTIVE", "MAINTENANCE", "OFFLINE"].includes(status)) {
        res.status(400).json({
          success: false,
          message:
            "Invalid status. Must be ACTIVE, INACTIVE, MAINTENANCE, or OFFLINE",
          statusCode: 400,
        });
        return;
      }
   ...
   */
  static async getLocations(req: AuthRequest, res: Response): Promise<void> {
    try {
