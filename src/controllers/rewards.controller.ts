  ...
  static async getMyBalance(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const totalOutstandingDebt = outstandingDebts.reduce((total, debt) => {
        const remaining =
          Math.abs(debt.currentAmount) - Math.abs(debt.paidAmount);
      ...
      if (currentBalance > 0 && totalOutstandingDebt > 0) {
        ...
        if (currentBalance >= totalOutstandingDebt) {
          ...
          finalOutstandingDebt = Math.max(
            0,
            totalOutstandingDebt - currentBalance
          );
          finalBalance = 0;
      ...
      const availableBalance =
        finalBalance - (pendingWithdrawals._sum.amount || 0);
