...
function getCachedPayments(userId: string) {
...
function setCachedPayments(userId: string, data: any) {
  paymentCache.set(userId, { data, timestamp: Date.now() });
}

function clearCachedPayments(userId: string) {
...
const initOnlinePaymentSchema = z
  ...
  .refine(
    (data) =>
      data.fineId || (data.fineIds && data.fineIds.length > 0) || data.debtId,
    {
...
export class PaymentController {
  ...
  static async getUserPayments(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      console.log(
        `✅ Fetched ${payments.length} payments in ${endTime - startTime}ms`
      );
   ...
   */
  static async initOnlinePayment(
    req: AuthRequest,
    res: Response
  ): Promise<void> {
    try {
      ...
      if (validatedData.debtId) {
        result = await sslCommerzService.createDebtPaymentSession(
      ...
      // Handle single fine payment
        ...
        result = await sslCommerzService.createFinePaymentSession(
          validatedData.fineId,
          userId,
          validatedData.amount,
          baseURL
        );
      ...
      // Handle multiple fines payment
        ...
        result = await sslCommerzService.createMultipleFinesPaymentSession(
          validatedData.fineIds,
          userId,
          baseURL
        );
      }
   ...
   */
  static async handleSSLSuccess(req: Request, res: Response): Promise<void> {
    try {
   ...
   */
  static async handleSSLFail(req: Request, res: Response): Promise<void> {
    try {
   ...
   */
  static async handleSSLCancel(req: Request, res: Response): Promise<void> {
    try {
   ...
   */
  static async handleSSLIPN(req: Request, res: Response): Promise<void> {
    try {
      const result = await sslCommerzService.handleSuccessCallback(req.body);
  ...
  static async initiateRefund(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const result = await sslCommerzService.initiateRefund(
        ...
        refundRemarks || "Refund requested"
      );
   ...
   */
  static async processDebtPayment(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { DebtManagementService } = await import(
        "../services/debtManagement.service"
      );
      ...
      const safePaymentAmount = Math.max(
        0,
        Math.min(paymentAmount, remainingDebt)
      );
      const updatedDebt = await DebtManagementService.recordPayment(
        debtId,
        safePaymentAmount,
        transactionId
      );
      ...
      if (updatedDebt.status === "PAID") {
        console.log(
          "✅ Debt fully paid, checking for any remaining balance issues"
        );
        ...
        await DebtManagementService.checkAndCreateDebtForNegativeBalance(
          debt.userId
        );
      }
      ...
      try {
        ...
        const remaining = Math.max(
          0,
          (updatedDebt.currentAmount || 0) - (updatedDebt.paidAmount || 0)
        );
        await emailService.sendDebtPaymentConfirmation({
          userEmail: debt.user?.email || "",
          userName: `${debt.user?.firstName || ""} ${
            debt.user?.lastName || ""
          }`.trim(),
          transactionId,
