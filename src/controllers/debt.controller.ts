...
export const getMyDebts = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const totalDebt = await DebtManagementService.getTotalDebtAmount(
      req.user.id
    );

    // Calculate total late fees
    const totalLateFees = debts.reduce(
      (sum: number, debt: any) => sum + debt.lateFees,
      0
    );
    ...
    const oldestDebt =
      debts.length > 0
        ? debts.reduce((oldest: any, current: any) =>
 ...
 */
export const getTotalDebt = async (req: AuthRequest, res: Response) => {
  try {
    if (!req.user?.id) {
      return res.status(401).json({ error: "Unauthorized" });
    }
 ...
 */
export const getDebtDetails = async (req: AuthRequest, res: Response) => {
  try {
    if (!req.user?.id) {
      return res.status(401).json({ error: "Unauthorized" });
    }
...
export const payDebt = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const { SSLCommerzService } = await import(
      "../services/sslcommerz.service"
    );
 ...
 */
export const waiveDebt = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const waivedDebt = await DebtManagementService.waiveDebt(
      debtId,
      req.user.id,
      notes
    );
 ...
 */
export const getAllDebts = async (req: AuthRequest, res: Response) => {
  try {
    if (!req.user?.id || req.user?.role !== "ADMIN") {
      return res.status(403).json({
        success: false,
