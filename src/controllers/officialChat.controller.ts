...
export const createOrGetChatRoom = async (
  ...
  try {
    ...
    const chatRoom = await OfficialChatService.createOrGetChatRoom(
      user1Id,
      user2Id
    );
...
export const sendMessage = async (
  ...
  try {
    ...
    const newMessage = await OfficialChatService.sendMessage(
      roomId,
      userId,
      message || "",
      messageType,
    ...
    const recipientId =
      chatRoom?.user1Id === userId ? chatRoom?.user2Id : chatRoom?.user1Id;
...
export const getMessages = async (
  ...
  try {
    ...
    const messages = await OfficialChatService.getMessages(
      roomId,
      userId,
      limit,
      skip
...
export const markMessageAsRead = async (
  ...
  try {
    ...
    const message = await OfficialChatService.markMessageAsRead(
      messageId,
      userId
    );
...
export const getAvailableOfficials = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
